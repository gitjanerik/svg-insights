...
## 2026-07-03 — v12.1.1: Ruteplanlegger-arket er drabar drawer + kritiske rettelser fra kodegjennomgang

Planlegg-/resultat-arket i Ruteplanleggeren er nå samme drabare drawer som turkartets infodrawer: åpner i 45 dvh, midtstilt håndtak drar mellom minimert (håndtak + kompakt «141 km · Grus 63 %»-linje) / standard / maksimert, og kartet er interaktivt bak når arket ikke er maksimert. I tillegg rettelser fra den adversarielle kodegjennomgangen av v12.1.0: **BRouter-profilene** hadde invertert «ukjent dekke»-logikk (utaggede distriktsveier fikk aldri grus-rabatten — nå får de den), ferge-linjen var død kode (ferger ble i praksis forbudt), og usikre lookup-navn (`motorcycle`, `locked`) som kunne få hele profilen avvist er fjernet; profil-opplastingen sjekker nå `error`-feltet i svaret (ugyldig profil ga forgiftet, cachet profileid) og hele profil-løypa dekkes av 20 s-timeouten. Ellers: tap på zoom-knappene satte A/B-veipunkt (interaktive barn ekskluderes nå fra kart-tap), rutehalen mot B kunne mangle visuelt (siste segment strekkes til enden), gjenåpning av lagret rute avbryter pågående ruting, veipunktene fryses ved beregnings-start, programmatisk satte søkefelter trigger ikke lenger stale Nominatim-dropdowns, dobbel-tap på «Lagre» dupliserer ikke, lagringsfeil vises, og en forbigående IndexedDB-feil låser ikke lenger all lagring resten av økten.
