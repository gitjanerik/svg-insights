...
## 2026-07-15 — v12.1.67: Rundtur-navigasjon + globalt søk-fallback

To turrettede forbedringer i kart-visningen. **(1) Gå en runde:** Stifinner har fått en «Gå en runde herfra» ved siden av «Naviger hit» i long-press-menyen. Det tapte punktet blir sløyfens origo (både start og mål); brukeren sikter deretter inn ett vendepunkt (via) med krysset, og ruten blir en ekte runde — ut én vei, hjem en annen — i stedet for tur/retur. Kjernen (`planLoop` i `routing.js`) gjenbruker rute-maskineriet: utturen origo → vendepunkt rutes som korteste vei, mens hjemveien søkes på en graf der utturens kanter er straffet, så den velger en annen sti tilbake. Blindvei faller pent tilbake til tur/retur, og motorvei ekskluderes som ellers. Flere vendepunkter kan legges til (opptil 3), og panelet viser «Rundtur» med total lengde + stigning/fall. **(2) Søk utenfor kartet:** kart-søket ser bare navn i det rendrede utsnittet, så et sted utenfor ga bare «ingen treff» — en lett felle å gå i. Nå faller søket tilbake til globalt stedssøk (Nominatim) når kart-søket er tomt: en «Andre steder i Norge»-seksjon dukker opp, og velger man et forslag bygges et nytt kart der. Placeholder og tom-tilstand er også klargjort så det er tydelig at søket gjelder dette kartet. 1145 tester grønne.

---

## 2026-07-09 — v12.1.66: Grid-tynning av dybdetall (gjeninnført) + bukt-fyll males ikke over sjøen

To fikser fra Trondheim-testing (Korsvika/Lade). **(1) Dybdetall-tynning:** Sjøkart-WFS leverer 5000+ dybdepunkt i tette havneområder, og ALLE ble emittert som tekst — long-press-lupen ble vegg-til-vegg med tall og ubrukelig. v7.1.13 hadde et 400 m-grid-filter («grunneste vinner» — mest sikkerhetsrelevant for kajakk), men det forsvant da sjøkart-modusen ble revet i v8.9.11 og ble aldri gjeninnført med resten i v8.9.16. Nå tilbake i to trinn: fin (120 m-celler) er alt som emitteres — lupe-tetthet og tak for SVG-bytes; grov (480 m-cellevinnerne) vises alltid når dybde-laget er på, mens resten (`data-fine`) CSS-gates til `.zoom-near` på hovedkartet. Lupen setter nå `.zoom-near`/`.zoomed-in` på sin egen SVG-rot og viser full fin-tetthet — det fikser også at kontur-tall og vann-moh var stylesheet-skjult i lupen. **(2) Korsvika-blåfargen:** OSM `natural=bay`/`strait`-polygoner (hentet for NAVNET på bukta/sundet) ble klassifisert som 303 saltvann og malt som flat mørkere flate OPPÅ den graderte Sjøkart-/DEM-sjøen — en hard polygon med rette kanter midt i fjorden. Når kartet har autoritativ sjø (DEM-0m/N50) hoppes bukt-/sund-fyllet nå over; navnet beholdes (sjo-navn/vann-label). Uten autoritativ sjø beholdes fyllet som før. Kart må regenereres for å få begge fiksene. 1141 tester grønne.

---

## 2026-07-09 — v12.1.65: MCP kart-søk, areal-bevisst geokoding og store kart

MCP-serveren fikk tre utvidelser som lukker gapet mot hva appen selv kan. (1) Nytt verktøy `sok_kart` gjenbruker appens søkelogikk (`buildSearchIndex`/`filterIndex` fra `useMapSearch`) headless: spesial-nøkkelordene «vann» (alle innsjøer inkl. navnløse, sortert på areal) og «topp» (høyeste topper sortert på moh) oppfører seg identisk med søkefeltet i appen, pluss fritekst-navnesøk. For at søket skal virke uten nettleser fikk `elementPosition` en geometri-fallback når `getBBox` mangler (linkedom har ingen), og tre `tagName`-sammenligninger ble gjort case-insensitive (linkedom gir UPPERCASE) — nettleser-oppførselen er uendret. (2) `sok_sted` rapporterer nå stedets utstrekning: bredde/høyde/areal i km² fra Nominatims bounding box (som allerede ble hentet, men forkastet) og en anbefalt `halfKm` som dekker hele stedet. (3) `bygg_kart` bygger store områder rent: `demResolutionForArea` velger grovere DEM/DOM-oppløsning (5→10→20→25 m) ut fra kart-arealet så WCS-forespørselen holder seg under et celletak i stedet for å time ut og falle stille tilbake til terrengløst kart; takgrensen er hevet til `halfKm` 20 (40×40 km), og `halfKm` er nå valgfri — utelates den ved bygging fra et stedsnavn, auto-dimensjoneres kartet til stedets utstrekning («hele Vestmarka»). Ren MCP-/lib-endring; appens runtime er uberørt (fallbacken treffer bare Node).
...
## 2026-07-08 — v12.1.58: Turrapport via MCP — berikelse langs rute (kulturminner/vern/arter) + samle-SVG

Trinnrakett i tre delte, testede libs som gjenbruker eksisterende fetchere, pluss to nye MCP-verktøy. (1) **`lib/routeEnrichment.js`** — beriker en rute med det som ligger LANGS den: fredede kulturminner (Riksantikvaren WFS), verneområder ruten går gjennom (Naturbase, parallelle punkt-oppslag med kort timeout), og rødlistede arter i korridoren (GBIF snittet mot Artsdatabankens norske rødliste fra `public/data/redlist-no.json`). Fetcherne injiseres → testbart uten nett; feil i én kilde svelges (graceful, `kilder`-flagg). (2) **`lib/routeCues.js`** — sti-kryss-varsler: finner der ruten svinger merkbart OG punktet er et faktisk kryss (node med grad ≥ 3 i rutenett-grafen), regner retning (venstre/høyre) og fester nærmeste navngitte holdepunkt fra kartet («ta til venstre ved Abbortjern»). (3) **`lib/tripReport.js`** — komponerer én selvstendig turrapport-SVG: kartutsnitt med ruten (nestet, isom-map-scoped), høydeprofil, funn-liste og veibeskrivelse. Nye MCP-verktøy **`berik_rute`** (strukturert JSON) og **`turrapport_svg`** (ferdig rapport-fil). Verifisert ende-til-ende: Bondivann → via Wentzelhytta → Vardåsen ga 1 kulturminne, 1 verneområde, 41 rødlistede arter og 16-stegs veibeskrivelse. 1077 lib/composable-tester passerer. Appens views er uendret (nye libs brukes kun av MCP-serveren).

---

## 2026-07-07 — v12.1.57: Via-punkter (0–3) i Stifinner + MCP, og roligere rute-strek

Bygger videre på v12.1.56. (1) **Via-punkter — delt kjerne:** ny `planRoutesThrough(rg, [start, ...via, mål])` i `lib/routing.js` ruter gjennom en ordnet punktliste (fast korteste prefiks innom hvert via-punkt, 1–3 alternativer på siste ledd). Brukt av både MCP-verktøyet `tegn_rute_svg` (ny `via`-parameter med valgfrie navn) og appens Stifinner. (2) **Via i Stifinner-UI (additivt):** A→B-flyten er uendret; når ruten vises kan man legge til inntil 3 via-punkter via samme sikte-crosshair (nå gult), «Bekreft via-punkt», og ruten reberegnes A → via… → B. Via-ene vises som gule prikker + chips med ✕, og ruten holdes synlig mens man sikter. `useStifinner` cacher routing-grafen så via-redigering ikke bygger grafen på nytt. (3) **Roligere rute-strek:** den anbefalte (valgte) ruta var hardkodet tykk og 100 % opak og druknet alternativene — nå er alle rute-streker semitransparente, og den valgte skiller seg kun med litt mer bredde/opasitet (`routeOverlay`-stilen). Nye enhetstester for `planRoutesThrough` og via-flyten i `useStifinner`; 1054 lib/composable-tester passerer.
...
## 2026-07-04 — v12.1.16: Ruteplanlegger — kartlag-toggles med parkering (turkart-regler) + «Åpne i UT.no» i begge kart

To nye lag-funksjoner i Ruteplanleggeren og én delt snarvei. **(1) Kartlag-panel:** tegnforklaringen nede til venstre er nå en lag-velger — «Bekreftet grus», «Antatt grus (skogsbilvei)» og ny «Parkering» kan skrus av/på hver for seg (valg huskes i localStorage; parkering er default av og hentes først når laget aktiveres). **(2) Parkering med turkartets regler:** samme regelsett som ISOM 534/534u — `amenity=parking` som node og way (centroid), utfartsparkering krever både offentlig access/utfarts-navn (`isTrailheadParking`) OG sti/skogsbilvei innen 50 m, og uttynning via `thinParkering` (utfart vises alltid, vanlige P min. 50 m — generalisert til zoombar visning med et 28 px-gulv). Regelsettet er flyttet til delt `parkingRules.js` (re-eksportert fra mapBuilder/symbolizer, alle eksisterende tester uendret); Overpass-spørringen henter kun sti-geometri innen 100 m av P-plassene (`around`), ikke alle stier i utsnittet. Utfarts-P tegnes med turkartets sorte hjørne-braketter (hvit halo). **(3) UT.no-lenke:** i Ruteplanleggeren åpner long-press (eller høyreklikk) en geo-ankret pin med koordinater og «Åpne i UT.no» — lenken (`ut.no/kart#zoom/lat/lon`, ny delt `utNoLink.js`) tar med gjeldende zoom så UT.no viser samme utsnitt; vanlig tap lukker pinnen uten å sette A/B. I turkartet ligger «UT.no-kart» som ny handling i long-press-arket ved siden av Google Maps/Street View, med zoom avledet av gjeldende bakkeoppløsning (inkl. pinch-zoom).
