...
Kart-SVG-en bruker **to ulike enhetssystemer**, og det er lett å blande dem:

1. **Geometri / koordinatrom = METER.** viewBox er `0 0 widthM heightM` i bakke-meter — `mapBuilder.js`: `// ViewBox = meter (1 SVG-enhet = 1 m)`. Et 5 km-kart har koordinater 0–5000 (store tall, ikke små). GPS-projeksjonen hardkoder det samme (`utm.js` `utmToSvg`/`svgToWgs84`).
2. **Symbol-/strek-/font-størrelser = mm (print-mm).** De små tallene (`0.05mm`, `0.07mm`, `2.6mm`) i `isomCatalog.json` («Alle dimensjoner i mm (print-kvalitet)», `intendedScale: 10000`). `mm` er en CSS-*absolutt* enhet (~3.78 user-units/mm), knyttet til fysisk papir — derfor er «2.6mm-label ≈ 26m» (2.6mm papir × 1:10000). Små **med hensikt** (ISOM print-spec).

**Ikke skaler koordinatrommet «×10» (mm→cm-idé).** Det er vurdert og forkastet: (a) de små tallene blir IKKE større — de er `mm` (absolutt), så `0.07mm` forblir `0.07mm` og rendres bare 10× tynnere relativt til kartet; (b) det er en ~dusin-steders refaktor der alt som antar «1 enhet = 1 m» ryker stille — GPS-punkt + long-press-oppslag (`utm.js`), detalj-inset (`DETAIL_INSET_M` + vindukonstanter i `MapView.vue`), bakte mm→meter-konverteringer (knaus `symUnit`, bro `broOffsetM`, veinummer `MM_TO_M`/`roadRefFontM` i `mapBuilder.js`), og `meta.widthM`-som-bakkemeter-konsumenter (ghost-tiles `/111320`, `strokeSizeBase`, ekstern-kart-zoom, nøyaktighetsring). Relieff-bitmap er derimot uproblematisk (SVG-innebygd PNG er oppløsnings-uavhengig; eksport følger fysisk `mm`-størrelse).
