...
const userMaps = ref([])
// v7.5.0 rebrand-migrering: skriv til ny nøkkel, men les både ny og gammel
// så turneringer som ble startet før rebrand-deploy fortsetter gjennom
// kart-bytter. Gjelder også 'curveball-autostart-mapId' (se readShareAutostart).
...
function consumeTournamentRestore() {
  try {
    ...
    // state+pendingNextMapId så vi ikke restorer dobbelt. Tøm legacy-key
    // ved samme anledning så den ikke forvirrer senere reads.
    sessionStorage.setItem(TOURNAMENT_KEY, JSON.stringify({
...
function onCurveBallRestart() {
  ...
  // så neste runde kan re-besøke samme kart. Også tøm legacy-key for
  // sikkerhet (v7.5.0 rebrand).
...
function consumeShareAutostart() {
  try {
    // v7.5.0: les ny + legacy-key for å støtte share-lenker som ble lagret
    // før rebrand-deploy. Begge ryddes etter konsumering.
