...
export function buildIsomCss(catalog = isomCatalogDefault, patternIds, options = {}) {
  ...
  // Inter variable webfont (https://rsms.me/inter/) som primær. font-weight: 400
  // er base for labels; spesifikke labels (peak, stedsnavn) overstyrer under.
  // tabular-nums sørger for at høyde-, dybde- og kontur-tall står monospaced.
  rules.push(`${root} { background: var(--bg, ${catalog.background.color}); font-family: 'InterVariable', 'Inter', ui-sans-serif, system-ui, sans-serif; font-weight: 400; font-variant-numeric: tabular-nums; }`)
